...
enum class Channels {
    PGY {
        override suspend fun getReleaseInfo(): ReleasesInfo {
            ...
            val appKey = "338591668ffe7afb774eedc8e5bd7f48"
            val url =
...
suspend fun getReleaseInfo(): ReleasesInfo? = withContext(Dispatchers.IO) {
    ...
    return@withContext list.firstOrNull()?.second
}

suspend fun checkUpdate(ativity: Activity, fromUser: Boolean = true) = withContext(Dispatchers.IO) {
    ...
    if (releasesInfo.isUpgradeable()) {
        val manager = DownloadManager.Builder(ativity).run {
            ...
            smallIcon(R.drawable.ic_watermark_camera_app)
